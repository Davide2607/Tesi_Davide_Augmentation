...
def valuta_modello(model, test_generator, run, model_name):
    ...
    log_extra_test_metrics(model, test_generator, run, model_name, 'finetuning')
...
def main():
    ...
    # Addestra il modello
    history = addestra_modello(model, train_generator, valid_generator, test_generator, FT_EPOCH, 10, 5, 0.003, 1e-6, run, model_name)
